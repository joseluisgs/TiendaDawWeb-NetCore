...
public class RatingService : IRatingService
{
        ...
        long usuarioId, long productoId, int puntuacion, string? comentario)
    {
        try
        {
            ...
            if (canRateResult.IsFailure)
            {
                return Result.Failure<Rating, DomainError>(canRateResult.Error);
            }

            if (!canRateResult.Value)
            {
                return Result.Failure<Rating, DomainError>(RatingError.ProductNotPurchased);
            }
            ...
            _logger.LogInformation("Usuario {UserId} valoró producto {ProductId} con {Puntuacion} estrellas", 
        ...
        {
            _logger.LogError(ex, "Error al añadir valoración para producto {ProductId}", productoId);
    ...
    public async Task<Result<IEnumerable<Rating>, DomainError>> GetByProductoIdAsync(long productoId)
    {
        ...
        {
            _logger.LogError(ex, "Error al obtener valoraciones del producto {ProductId}", productoId);
    ...
    public async Task<Result<Rating, DomainError>> GetByIdAsync(long id)
    {
        ...
        {
            _logger.LogError(ex, "Error al obtener valoración {RatingId}", id);
            return Result.Failure<Rating, DomainError>(
        ...
        long ratingId, long usuarioId, int puntuacion, string? comentario)
    {
        try
        {
            ...
            if (rating == null)
            {
                return Result.Failure<Rating, DomainError>(RatingError.NotFound(ratingId));
            }
            ...
            _logger.LogInformation("Valoración {RatingId} actualizada por usuario {UserId}", 
                ratingId, usuarioId);
        ...
        {
            _logger.LogError(ex, "Error al actualizar valoración {RatingId}", ratingId);
            return Result.Failure<Rating, DomainError>(
    ...
    public async Task<Result<bool, DomainError>> DeleteRatingAsync(long ratingId, long usuarioId, bool isAdmin = false)
    {
        try
        {
            ...
            _logger.LogInformation("Valoración {RatingId} eliminada por usuario {UserId}", 
                ratingId, usuarioId);
        ...
        {
            _logger.LogError(ex, "Error al eliminar valoración {RatingId}", ratingId);
    ...
    public async Task<Result<double, DomainError>> GetAverageRatingAsync(long productoId)
    {
        ...
        {
            _logger.LogError(ex, "Error al calcular promedio de valoraciones para producto {ProductId}", productoId);
    ...
    public async Task<Result<bool, DomainError>> CanUserRateProductAsync(long usuarioId, long productoId)
    {
        ...
        {
            _logger.LogError(ex, "Error al verificar si usuario {UserId} puede valorar producto {ProductId}", 
