...
Log.Information("🚀 Inicializando TiendaDawWeb (Razor Pages)...");
...
// Core - Razor Pages y Blazor (sin MVC Controllers)
...
{
    var urls = configuration["ASPNETCORE_URLS"]?.Split(';') ?? new[] { "http://localhost:5002" };
    var firstUrl = urls.FirstOrDefault() ?? "http://localhost:5002";
    ...
    var port = firstUrl.Contains(':') ? firstUrl.Split(':').Last() : "5002";

    var mode = isDevelopment ? "DESARROLLO" : "PRODUCCION";
    var baseUrl = $"{protocol}://{host}:{port}";

    Log.Information("=================================================================");
    Log.Information("TiendaDawWeb - Aplicación Web Educativa (RAZOR PAGES)");
    Log.Information("=================================================================");
